...
class Icinga2ReqSensor(Sensor):
  ...
  def sfilter(self,payload):
    for f in self.filter:
      ...
      if atype == 'exists':
        if key in payload['var']: return True
      if atype == 'nexists':
        if key not in payload['var']: return True
      if key in payload:
        if atype == 'equals':
          if payload[key] == pattern: return True
        if atype == 'nequals':
          if payload[key] != pattern: return True
        if atype == 'contains':
          if payload[key].find(pattern): return True
        if atype == 'ncontains':
          if not payload[key].find(pattern): return True
      elif key in payload['var']:
        if atype == 'equals':
          if payload['var'][key] == pattern: return True
        if atype == 'nequals':
          if payload['var'][key] != pattern: return True
        if atype == 'contains':
          if payload['var'][key].find(pattern): return True
        if atype == 'ncontains':
          if not payload['var'][key].find(pattern): return True
    return False
